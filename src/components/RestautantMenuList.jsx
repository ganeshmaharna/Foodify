...
const RestaurantMenuList = ({ items }) => {
  ...
  return (
    <div className='accordion-body'>
      {
        items?.map((item) => (
          <div key={item?.card?.info?.id} className='item flex items-start justify-between pb-8 my-2 mx-2 border-b-2 border-gray-200'>
            <div className='md:w-auto w-3/5'>
              {
                item?.card?.info?.itemAttribute?.vegClassifier === 'VEG' ? <FaRegCircleDot style={{ color: "green" }}/> : <FaRegCircleDot style={{ color: "red" }}/>
              }
              <h4 className='text-base font-poppins'>{item?.card?.info?.name}</h4>
              {
                item?.card?.info?.price ? <span className='rupee font-poppins text-sm'>{item?.card?.info?.price / 100}</span> : <span className='rupee font-poppins text-sm'>{item?.card?.info?.defaultPrice / 100}</span>
              }
              {
                item?.card?.info?.description && <p className='text-gray-500 mt-3  text-md md:w-3/4 font-poppins'>{item?.card?.info?.description}</p>
              }
            </div>
